...
<body class="layui-anim layui-anim-up">
	...
	<div class="x-body">
		<div class="layui-row">
			<form class="layui-form layui-col-md12 x-so" action="/Customer/showAllServe">
		...
		<xblock>
		<button class="layui-btn" id="return1" ailin="22" onclick="window.location.href='/Customer/showNoAssigneServe' ">返回</button>
			<button ailin="14" id="adlbtn" class="layui-btn">
				<i class="layui-icon"></i>创建服务
			</button>
			<button ailin="14" id="new" class="layui-btn">
				<i class="layui-icon"></i>新客来访
			</button>
			...
			<tbody id="pg">
				<c:forEach items="${list}" var="Serve" varStatus="ser">
					<tr>
						...
						<c:forEach items="${listCus}" var="cusname" varStatus="cus">
		...
		layui.use(['laypage', 'layer'], function() {
			...
			laypage.render({
				...
				jump: function(obj, first) {
					//首次不执行
				    if(!first){
						location.href="/Customer/showAllServe?currentPage="+obj.curr;
				    }
	...
	$("#new").click(function() {
		...
		layer.open({
			...
			maxmin : true,
			closeBtn : 1,
			content : [ './newserve.jsp', 'yes' ], //iframe的url，yes是否有滚动条
			//yes: function (index, layero) {
			//    alert(index);
			//    alert(layero);
			//},
