...
<body class="layui-anim layui-anim-up">
	...
	<div class="x-body">
		<div class="layui-row">
			<form class="layui-form layui-col-md12 x-so" action="/Customer/showNoSersatisfy">
		...
		<c:if test="${not empty listSatisfy}">
		<table class="layui-table">
			...
			<tbody>
				<c:forEach items="${listSatisfy}" var="Serve">
					<tr>
						<td>${Serve.serveid}</td>
	...
	<script type="text/javascript">
	 function member_del(obj, id) {
			...
			layer.open({
				...
				maxmin : true,
				closeBtn : 1,
				content : [ './modifySatisfy.jsp?serveid='+id, 'yes' ], //iframe的url，yes是否有滚动条
				end : function() {
					location.reload();
				}
			});
		...
		layui.use(['laypage', 'layer'], function() {
			...
			laypage.render({
				...
				jump: function(obj, first) {
					//首次不执行
				    if(!first){
						location.href="/Customer/showNoSersatisfy?currentPage="+obj.curr;
				    }
